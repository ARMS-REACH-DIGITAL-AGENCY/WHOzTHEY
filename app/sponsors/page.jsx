    ...
    <main className="wt-app">
      ...
      <header className="wt-top">
        ...
        <section className="wt-input-card">
          ...
          <textarea
            value={activeTab === 'website' || activeTab === 'hooks' || activeTab === 'story' || activeTab === 'offer' || activeTab === 'package' ? website : claim}
            onChange={(event) => {
              if (activeTab === 'website' || activeTab === 'hooks' || activeTab === 'story' || activeTab === 'offer' || activeTab === 'package') setWebsite(event.target.value)
              else setClaim(event.target.value)
            }}
            placeholder={activeTab === 'website' || activeTab === 'hooks' || activeTab === 'story' || activeTab === 'offer' || activeTab === 'package' ? 'Enter sponsor website...' : 'Enter a They say claim...'}
          />
          <button type="button" className="wt-go" onClick={activeTab === 'website' || activeTab === 'hooks' || activeTab === 'story' || activeTab === 'offer' || activeTab === 'package' ? analyzeWebsite : runClaimDemo} disabled={loading}>
            {loading ? '...' : activeTab === 'website' || activeTab === 'hooks' || activeTab === 'story' || activeTab === 'offer' || activeTab === 'package' ? 'SCAN' : 'ASK'}
          </button>
        </section>
      ...
      <section className="wt-content">
        ...
        {activeTab === 'origin' && <OriginPanel claim={claim} setClaim={setClaim} runClaimDemo={runClaimDemo} claimResult={claimResult} />}
...
function OriginPanel({ claim, setClaim, runClaimDemo, claimResult }) {
  return (
...
function labelFor(key) {
  return {
    whyItWorks: 'Why it works', bestFor: 'Best for', story: 'Story', emotionalTrigger: 'Emotional trigger', suggestedCTA: 'CTA', why: 'Why'
  }[key] || key
}

const css = `
  :root{--navy:#0f1726;--navy2:#172235;--red:#dc2626;--cream:#f9f3e9;--gold:#f6c453;--muted:#768195}
