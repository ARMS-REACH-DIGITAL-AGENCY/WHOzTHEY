...
// Unambiguous alphabet (no 0/O/1/l/I) for short share-link codes.
...
// Idempotent — safe to call before every claims write.
export async function ensureClaimShortCodeColumn(sql) {
  await sql`alter table claims add column if not exists short_code text`
  await sql`create unique index if not exists claims_short_code_idx on claims (short_code) where short_code is not null`
}
